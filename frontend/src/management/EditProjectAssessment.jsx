...
const EditProjectAssessment = () => {
  ...
  const fetchAssessmentData = async () => {
    try {
      const response = await axios.get(`/api/management/project-assessments/${id}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (formData[key] !== null && key !== 'resources') {
          formDataToSend.append(key, formData[key]);
        }
      });

      if (formData.resources instanceof File) {
        formDataToSend.append('resources', formData.resources);
      }

      await axios.put(`/api/management/project-assessments/${id}/`, formDataToSend, {
        headers: {
      ...
      setError('Failed to update project assessment. Please try again.');
      console.error('Error:', error);
    } finally {
      setSaving(false);
    }
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-2">
                    Resources
                    {formData.resources && !formData.resources instanceof File && (
                      <span className="text-sm text-gray-500 ml-2">(Current: {formData.resources})</span>
                    )}
                  </label>
                  <input
                    type="file"
...
export default EditProjectAssessment; 